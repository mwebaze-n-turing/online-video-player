// VideoPlayerControlsEnhanced.tsx
import { useState } from 'react';
import EnhancedVolumeControl from './EnhancedVolumeControl';

interface VideoPlayerControlsEnhancedProps {
  isPlaying: boolean;
  isMuted: boolean;
  volume: number;
  currentTime: number;
...
export default function VideoPlayerControlsEnhanced({ 
  isPlaying, 
  isMuted, 
  volume,
  currentTime,
...
}: VideoPlayerControlsEnhancedProps) {
  const [speedMenuOpen, setSpeedMenuOpen] = useState(false);
  const [settingsMenuOpen, setSettingsMenuOpen] = useState(false);
  
  return (
    <div className={`control-bar absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3 transition-all duration-300 ${
